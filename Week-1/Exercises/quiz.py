...
def _normalize(text, case_sensitive=True):
    ...
    # unify CRLF -> LF
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    # convert typed literal sequences like '\n' and '\t'
    text = text.replace("\\n", "\n").replace("\\t", "\t")
    text = text.replace('"', "'")
...
def _read_multiline_answer():
    """
    Read multiline user input until a blank line or EOF.
    Usage instruction is printed to the user.
    """
...
def quiz_user(code_snippet, *, case_sensitive=True):
    """
    Display code, ask question, and check the user's answer.
    """
    print("\n--- Code snippet ---\n")
    print(code_snippet)
...
def run_quiz_from_csv(filename, section = "Warm-up", start_row = 0):
    ...
    Each row must have 'section' and 'code' columns.
    ...
    with open(filename, newline='', encoding='utf-8') as csvfile:
        ...
        if os.path.exists(results_file):
            try:
                ...
                # If reading/parsing fails, fall back to starting fresh
                append_mode = False
                resume_start_row = start_row
        ...
        # Now read the full results file (all sessions) and count correct answers
        with open(results_file, newline='', encoding='utf-8') as rf:
            existing = list(csv.DictReader(rf))
